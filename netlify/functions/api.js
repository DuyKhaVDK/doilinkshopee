...
// --- SỬA LỖI TRUY XUẤT HÀM DỰA TRÊN LOG THỰC TẾ ---
const netlifyBlobs = require('@netlify/blobs');

// Truy xuất getStore dựa trên danh sách hàm [ 'Blobs' ] bạn đã cung cấp
const getStore = netlifyBlobs.getStore || 
                 (netlifyBlobs.Blobs && netlifyBlobs.Blobs.getStore) || 
                 (netlifyBlobs.default && netlifyBlobs.default.getStore);
...
// Kiểm tra trạng thái hệ thống ngay khi khởi động
if (typeof getStore !== 'function') {
    console.error("⚠️ LỖI: Không tìm thấy getStore. Cấu trúc:", JSON.stringify(Object.keys(netlifyBlobs)));
} else {
    console.log("✅ THÀNH CÔNG: Hệ thống lưu trữ Blobs đã sẵn sàng.");
}
...
router.post('/convert-text', async (req, res) => {
    ...
    if (successCount > 0 && typeof getStore === 'function') {
        try {
            ...
            console.error("Lỗi cập nhật số liệu:", e.message);
        }
    }
...
// --- ROUTER 2: XEM THỐNG KÊ (CHỈ CHO ADMIN) ---
router.get('/admin/stats', async (req, res) => {
    ...
    try {
        if (typeof getStore !== 'function') {
            throw new Error("Dịch vụ lưu trữ (Blobs) chưa khởi tạo thành công.");
        }
