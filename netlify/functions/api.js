...
router.post('/convert-text', async (req, res) => {
    ...
    if (successCount > 0) {
        try {
            ...
            // Lấy toàn bộ dữ liệu để tính toán
            const dbRes = await axios.get(`${DB_ROOT}.json`);
            const dbData = dbRes.data || {};
            const currentStats = dbData.stats || {};
            const currentDaily = dbData.daily?.[today] || 0;

            let newTotal = (currentStats.last_date !== today) ? successCount : (currentStats.total_converted || 0) + successCount;

            // Ghi đè đồng thời vào stats và daily để đảm bảo xuất hiện mục daily
            const updates = {
                stats: {
                ...
                [`daily/${today}`]: currentDaily + successCount
            };
...
// --- ROUTER ADMIN: TRẢ VỀ CẢ STATS VÀ DAILY ---
router.get('/admin/stats', async (req, res) => {
    const token = req.headers['x-admin-token'];
    if (!token || token !== ADMIN_SECRET) return res.status(403).json({ success: false, error: 'Mã bí mật không đúng!' });
    ...
    try {
        ...
        res.json({
            ...
            daily: dbData.daily || {} // Gửi thêm dữ liệu ngày về frontend
        });
